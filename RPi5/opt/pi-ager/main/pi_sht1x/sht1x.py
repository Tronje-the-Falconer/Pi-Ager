...
try:
    ...
    if ('Raspberry Pi 5' in output):
        import RPi.GPIO as GPIO     # will not work because rpi-lgpio does not allow shared gpio pin access
        # cl_fact_logger.get_instance().info("Pi5 detected")
    else:
        import main.RPi.GPIO as GPIO    # old style RPi.GPIO will work on all other RPi, but not on RPi 5 with RP1 chip
