        ...
        if ($modus_setting != NULL) {             // ist das $_POST-Array gesetzt
            ...
            if ($SettingsInputIsValid == TRUE) {
                if ( $setpoint_temperature_setting <= 70 &&  $setpoint_temperature_setting >= -11 &&        // Prüfung Soll-Temperatur
                    ...
                    $res = eval_cooler_heater_offsets( $temperature_control_params, $setpoint_temperature_setting );
                    $cooling_hysteresis_offset = round($res[0], 1);
                    $heating_hysteresis_offset = round($res[1], 1); 
                    write_temperature_control_offsets($cooling_hysteresis_offset, $heating_hysteresis_offset);  
