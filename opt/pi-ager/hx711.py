...
class HX711:
    def __init__(self, dout=10, pd_sck=9, gain=128, bitsToRead=24):
        ...
        Args:
            dout(int): Raspberry Pi pin number where the Data pin of HX711 is connected.
            pd_sck(int): Raspberry Pi pin number where the Clock pin of HX711 is connected.
            gain(int): Optional, by default value 128. Options (128 || 64)
            select_channel(str): Optional, by default 'A'. Options ('A' || 'B')

        Raises:
            TypeError: if pd_sck or dout are not int type
        """
        if (isinstance(dout, int)):
            if (isinstance(pd_sck, int)):
                ...
                raise TypeError('pd_sck must be type int. '
                                'Received pd_sck: {}'.format(pd_sck))
        else:
            raise TypeError('dout_pin must be type int. '
                            'Received dout: {}'.format(dout))
    ...
    def _set_channel_gain(self, num):
        ...
        for _ in range(num):
            ...
            if end_counter - start_counter >= 0.00006:
                ...
                # result = self.get_raw_data_mean(6)  # set for the next reading. This function is called also from _read()
                                                    # which in turn calls this funktion gain, endless loop!
                # if result == False:
                # return False
        return True
    ...
    def getValue(self):
        return( self.get_raw_data_mean(6) - self.OFFSET )
    ...
    def tare(self, times=25):
        ...
        # remove spikes
        cut = times//5
        values = sorted([self.getValue() for i in range(times)])[cut:-cut]
        offset = stat.mean(values)
