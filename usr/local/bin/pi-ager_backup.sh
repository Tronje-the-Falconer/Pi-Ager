...
set_piager_status () {
    if [ $PIAGER_STATUS == 1 ]; then
        log "Set pi-ager active in DB"
        start_piager
    fi
    if [ $SCALE1_STATUS == 1 ]; then
        log "Set scale1 active in DB"
        start_scale1
    fi
    if [ $SCALE2_STATUS == 1 ]; then
        log "Set scale2 active in DB"
        start_scale2
    fi
}
...
log "current PID:  $currpid"
log "Running PID : $runpid"
if [[ $runpid != $currpid ]]
	then
	  log "Another backup process is already running! Current backup process stopped."
	  exit 1
fi	 
...
log "Main Status is $MAIN_STATUS"
...
log "ok let's go $(date +%T)"
anfang=$(date +%s)

log "Backup is starting, it will need some time to complete ..."

# Überprüfen ob Agingtable aktiv ist
log "check if aging_table is active"
log "agingtable status is ${AGINGTABLE_STATUS}"
if [ $AGINGTABLE_STATUS -ne "1" ]
    then
	  log "Agingtable is not active. Backup continues!"
	else
  	  log "Agingtable is active. Backup stopped!"
	  exit 1
fi	


#Überprüfen ob der NFS-Server vorhanden ist
log "check if nfs directory exists in the backup table."
if [ -z "$NFSVOL" ]
	then
        log "backup not setup correctly. Please enter nfs directory in the backup table! Format example is <ip address>:/folder/subfolder"
        exit 1
    else
        log "$NFSVOL exists. Backup continues!"
fi

log "check if format of nfs directory is correct, e.g.: 192.168.0.111:/srv/backup"

array=(${NFSVOL//:/ })
serverIP=${array[0]}
log "NFS IP address = $serverIP"
localIP=$(hostname -I)
...
log "check if nfs Server IP address has correct format"
if [[ "$serverIP" =~ ^(([1-9]?[0-9]|1[0-9][0-9]|2([0-4][0-9]|5[0-5]))\.){3}([1-9]?[0-9]|1[0-9][0-9]|2([0-4][0-9]|5[0-5]))$ ]]; then
  log "nfs Server address $serverIP has correct format"
else
  log "nfs Server address $serverIP is not defined or has wrong format. Backup stopped."
  exit 1
fi

log "check if nfs Server IP and local IP are different."
if [[ "$localIP" == "$serverIP" ]]; then
  log "nfs Server IP and local IP are equal. Backup stopped."
  exit 1
else
  log "nfs Server IP and local IP are different. Backup continues."
fi

log "check if default NFS mount point $NFSMOUNT exists in local filesystem."
if [ -d "$NFSMOUNT" ]
	then
		log "$NFSMOUNT exists. Backup continues."
	else
		log "$NFSMOUNT missing, will be be created."
		mkdir -p $NFSMOUNT
		chmod -R u=rwx,g+rw-x,o+rwx $NFSMOUNT
		log "$NFSMOUNT is created."								 
fi

DIR=$NFSMOUNT
 
log "Start $BACKUP_NAME $(date +%T)"
log "NFSVOL=$NFSVOL"
log "NFSMOUNT=$NFSMOUNT"
log "NFSOPT=$NFSOPT"
log "Backup file ${BACKUP_PFAD}/${BACKUP_NAME}.img"

# Vorsichtshalber einmal unmounten
umount $NFSMOUNT

# NFS-Volume mounten
log "mount NFS-Volume. Map $NFSVOL to $NFSMOUNT"

if [ -n "$NFSOPT" ]
	then
        log "mount with options: $NFSOPT"
		mount -t nfs $NFSVOL $NFSMOUNT -o "${NFSOPT},nfsvers=3"
        mountstatus=$?
 	else
        log "mount w/o options"
 		mount -t nfs $NFSVOL $NFSMOUNT -o nfsvers=3
        mountstatus=$?
fi

if [ $mountstatus -ne 0 ]; then
  log "Error $mountstatus during mount NFS Volume $NFSVOL. Backup stopped."
  exit 1
else
  log "mount NFS-Volume $NFSVOL successfull. Backup continues."
fi

# prüfe, ob current user in das Verzeichnis schreiben kann

u="$USER"
log "check if $u can write into $DIR mapped to $NFSVOL"
...
if [[ "$ACCESS" == "no" ]]; then
      log "$u has no WRITE permission. Backup stopped."
      log "Can be fixed by using command chmod, or"
      log "adjust OWNER and GROUP by using command chown."
      log "OWNER = $OWNER"
      log "GROUP = $GROUP"
      log "PERMISSION = $PERM"
      umount $NFSMOUNT
      ...
      log "$u can write into $DIR mapped to $NFSVOL, Backup continues."
fi
...
log "create now Backup ${BACKUP_PFAD}/${BACKUP_NAME}.img at $(date +%T) with command dd. This needs some time to complete ..."
dd if=/dev/mmcblk0 of=${backupfullname}.img bs=1M 2>&1

ddstatus=$?
if [ $ddstatus -ne 0 ]; then
  log "error $ddstatus during dd command. Backup stopped."
  if [ $PI_AGER_MAIN_ACTIVE == 1 ]; then
#    set_piager_status
    log  "Start Pi-Ager Main service again."
...
log "Final backup name is ${new_backup_name}"
...
log "Number of backups that are kept. ${BACKUP_ANZAHL}"
log "Actual number of backups. ${NUMBER_OF_BACKUPS}"

if [ ${NUMBER_OF_BACKUPS} -gt ${BACKUP_ANZAHL} ]
    then
        log "more than ${BACKUP_ANZAHL} backup files, some old backups will be removed"
        ...
        log "less or equal than ${BACKUP_ANZAHL} backup files, no backups will be removed"
fi
...
if [ $diff -lt 60 ]; then
    log " Backup successful after $diff seconds"

# Wenn kleiner 3600 Sekunden, in Minuten und Sekunden umrechnen
#################################################################
elif [ $diff -lt  3599 ]; then
   log " Backup successful after $(($diff / 60)) minute(s) $((diff % 60)) seconds"

# Wenn gleich oder groeßer 3600 Sekunden, in Stunden Minuten und Sekunden umrechnen
#################################################################
elif [ $diff -ge 3600 ]; then
   log " Backup successful after $((diff / 3600)) hour(s) $((diff % 3600 / 60)) minutes $((diff % 60)) seconds"
fi

log "shrink now image, add 100MB extra space"
...
umount -l $NFSMOUNT
if [ $? -ne 0 ]; then
    log "umount $NFSMOUNT failed. Exit now"
    exit 1
fi
